...
class Server(base.BaseV21):
    ...
    def get(self, name, **kwargs):
        param_name = "host" if self.is_valid_ip(name) else "name"

        return self._post("slb.server.search", {param_name: name}, **kwargs)
    ...
    def update(self, name, ip_address, **kwargs):
        ...
        if "port_list" in kwargs:
            params["server"]["port_list"] = kwargs["port_list"]

        self._post("slb.server.update", params, **kwargs)

    def fetchStatistics(self, name, **kwargs):
        param_name = "host" if self.is_valid_ip(name) else "name"
        return self._post("slb.server.fetchStatistics", {param_name: name},
                          **kwargs)

    def delete(self, name, **kwargs):
        param_name = "host" if self.is_valid_ip(name) else "name"
        self._post("slb.server.delete", {"server": {param_name: name}}, **kwargs)
